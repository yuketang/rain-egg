...
const locales = function (app, options) {
  app.logger.info('=================================app, options', app, options)
  ...
  for (let i = 0; i < localeDirs.length; i++) {
    ...
    for (let j = 0; j < names.length; j++) {
      const name = names[j];
      app.logger.info('=================================1111', 1111)

      const filepath = path.join(dir, name);
      app.logger.info('=================================2222',2222);
  ...
  app.context.__getLocale = function () {
    ...
    const headerLocale = this.headers.get(headerField);
...
module.exports = function(options ,app) {
  ...
  app.config.rainI18n.functionName = '__';

  /* istanbul ignore next */
  app.config.rainI18n.dirs = Array.isArray(app.config.rainI18n.dirs) ? app.config.rainI18n.dirs : [];
  ...
  for (const unit of app.loader.getLoadUnits()) {
    ...
    if (fs.existsSync(localePath)) {
      app.config.rainI18n.dirs.push(localePath);
    } else {

      app.config.rainI18n.dirs.push(path.join(unit.path, 'config/locales'));

    }
  }

  debug('app.config.rainI18n.dirs:', app.config.rainI18n.dirs);

  locales(app, app.config.rainI18n);
  ...
  // 自动加载 Middleware
  app.config.coreMiddleware.push('rainI18n');
};
